...
import pupil_labs.map_fixations_on_face.draw_landmarks as draw
...
def run_all(args_input):
    ...
    plot_all= args_input.get("plot_all")
    circle = args_input.get("aoi_radius")
    ...
    events_df = pd.read_csv(
        Path(raw_data_folder, "events.csv"), dtype=oftype
    )

    gaze_df = pd.read_csv(
        Path(raw_data_folder, "gaze.csv"), dtype=oftype
    )

    # Merge the two sources of fixation data
    fixations_on_face = pd.read_csv(
        Path(face_folder, "fixations_on_face.csv"), dtype=oftype
    )
    selected_col = ['fixation id', 'fixation on face']
    fixation_on_face_TF = fixations_on_face[selected_col]

    fixations_raw = pd.read_csv(
        Path(raw_data_folder, "fixations.csv"), dtype=oftype
    )

    fixations_all = pd.merge_asof(
        fixations_raw,
        fixation_on_face_TF,
        on="fixation id",
        direction="nearest",
   ...
   # Map fixations:
    mapped_fixations = map_fixations.map_fixations_on_face(fixations_all, face_df, circle, ellipse)

    logging.info("Merging dataframes")
    face_df = face_df.sort_values(by="timestamp [ns]")
    mapped_fixations['timestamp [ns]'] = mapped_fixations['fixation_timestamp'].astype('uint64')
    mapped_fixations=mapped_fixations.sort_values(by="timestamp [ns]")
    ...
    if start != "recording.begin":
        ...
        if not events_df["name"].isin([start]).any():
            ...
            start = events_df[events_df["name"] == start]["timestamp [ns]"].values[
                0
            ]
    ...
    if output_path is None:
        ...
        out_csv = output_file.replace(
            os.path.split(output_file)[1], "merged_data.csv"
        )
        output_path = os.path.split(output_file)[0]
    else:
        output_file = os.path.join(output_path, "fixation-on-face.mp4")
      ...
      with av.open(output_file, "w") as out_container:
          ...
          with Progress() as progress_bar:
              ...
              while num_processed_frames < merged_video.shape[0]:
                  ...
                  frame = frame[:, :, :]
                  xy = row['fixation coordinates']
                  xyg = row[["gaze x [px]", "gaze y [px]"]].to_numpy(dtype=np.int32)
                  xy = np.array(xy, dtype=np.int32)
                  fixation_id = row['fixation_id']
                  landmark = row['landmark']

                  # Draw bounding box and facial landmarks
                  draw.draw_face_landmarks(frame, row, circle, ellipse)
                  ...
                  # make a circle on the fixations
                  if not np.isnan(xy).any():
                      cv2.circle(frame, xy, int(circle), (255, 0, 0), 10)
                      text = f"{fixation_id}"
                      cv2.putText(frame, text, (xy[0] + 20, xy[1] - 20),
                      cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)

                      text_landmark = f"{fixation_id, landmark}"
                      text_location = (frame.shape[1] - 300, 50)
                      text_box_size = (300, 50)

                      # Create a black box, put text inside
                      cv2.rectangle(frame, text_location, (text_location[0] + text_box_size[0], text_location[1] + text_box_size[1]), (0, 0, 0), -1)
                      cv2.putText(frame, text_landmark, (text_location[0] + 10, text_location[1] + 35),
                      cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
          ...
          df_unique = merged_video.drop_duplicates(subset=['fixation_id'])
          df_unique.to_csv(os.path.join(output_path, 'mapped_fixations_on_face.csv'), index=False, sep='\t')
          logging.info(f"CSV files saved at: {out_csv}")

          logging.info(
              "[white bold on #0d122a]◎ Analysis and rendering completed! ⚡️[/]",
              extra={"markup": True},
          )
...
def run_main():
    root = tk.Tk()
    root.title("Map fixations on facial landmarks")
    ...
    tk.Button(root, text="Browse", command=lambda: browse_directory(face_mapper_output_folder)).pack()
    ...
    # Checkbox for plot_gaze
    plot_gaze_var = tk.BooleanVar()
    tk.Checkbutton(root, text="Do you want to plot both gaze and fixation?", variable=plot_gaze_var).pack()
